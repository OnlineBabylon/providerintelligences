...
import { getStateFullName, getSpecialtyFullName, getRelatedSpecialties } from '../../utils/dataMappings';

export default function ProviderPage({ stateCode, specialtyCode }) {
  ...
  const specialtySlug = specialtyCode.toLowerCase();
  const relatedSpecialties = getRelatedSpecialties(specialtyCode);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Helmet>
        ...
        <meta 
          name="description" 
          content={`Access a comprehensive database of ${stateName} ${specialtyName.toLowerCase()} providers to supercharge your healthcare sales and marketing efforts. Connect with ${specialtyName.toLowerCase()} decision-makers today.`} 
        />
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
              ...
              {loading ? (
                ...
                <>
                  ...
                  {/* Email Collection Modal - Same as original */}
                </>
              )}
            </div>
          </div>
      ...
      <div className="mt-12 bg-gray-50 p-6 rounded-lg">
        <h2 className="text-xl font-bold mb-4">Related Healthcare Specialties in {stateName}</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {relatedSpecialties.map(specialty => (
            <Link key={specialty.code} to={`/${stateSlug}/${specialty.slug}`} className="text-blue-600 hover:underline">
              {specialty.name} ({specialty.count.toLocaleString()}+ providers)
            </Link>
          ))}
        </div>
      </div>
...
export async function getStaticProps({ params }) {
  ...
  try {
    // Fetch top providers for the preview
    const response = await axios.get(
      `https://docapi-uax3j.ondigitalocean.app/api/providers/search?state=${state}&specialty=${specialty}&limit=10`
    );
    ...
    return {
      props: {
        ...
        initialProviders: response.data.providers || [],
    ...
    console.error(`Error fetching data for ${state}/${specialty}:`, error);
    return {
      props: {
        stateCode: state,
        specialtyCode: specialty,
...
// Generate all valid state-specialty combinations
export async function getStaticPaths() {
  try {
    // Fetch all valid states and specialties
    const response = await axios.get('https://docapi-uax3j.ondigitalocean.app/api/providers/filters');
    
    // Filter to only include valid combinations (implementation depends on API)
    // This is a placeholder - actual implementation would need to check which combinations have providers
    const validPaths = [
      // Priority paths (most common combinations)
      { params: { state: 'TX', specialty: '207RC0000X' } }, // Texas Cardiology
      { params: { state: 'CA', specialty: '207RC0000X' } }, // California Cardiology
      { params: { state: 'NY', specialty: '207RC0000X' } }, // New York Cardiology
    ...
    // Return at least the Texas Cardiology path that we know works
    return {
      paths: [{ params: { state: 'TX', specialty: '207RC0000X' } }],
      fallback: 'blocking',
    };
  }
} 